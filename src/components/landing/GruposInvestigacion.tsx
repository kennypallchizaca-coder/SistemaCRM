...
import { GRUPOS_INVESTIGACION } from '../../data/landing.data';
import { CAROUSEL_CONFIG, INSTITUTION } from '../../config/constants';
  ...
  return (
    <section id="investigacion" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Carrusel de Grupos de Investigación */}
        <div className="relative">
            ...
            {GRUPOS_INVESTIGACION.map((item) => (
              <div key={item.id} className="snap-start shrink-0 w-[88vw] sm:w-[320px] lg:w-[380px] bg-white rounded-none shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col group">
                <div className="h-48 sm:h-56 overflow-hidden relative">
                  ...
                  <div className="absolute top-4 right-4 bg-ups-yellow w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center shadow-md">
                    {getIconForGrupo(item.id)}
                  </div>
                </div>
                <div className="p-5 sm:p-8 flex flex-col flex-grow bg-white border-t-4 border-ups-yellow">
                  <h3 className="text-lg sm:text-xl font-bold text-ups-blue mb-2 sm:mb-3 uppercase">{item.title}</h3>
                  <p className="text-gray-600 text-sm sm:text-base mb-6 sm:mb-8 flex-grow">{item.description}</p>
          ...
          <button
            onClick={() => scroll('right')}
            className="hidden sm:flex absolute -right-4 lg:-right-12 top-1/2 -translate-y-1/2 z-20 p-2 sm:p-3 bg-white border border-gray-200 text-ups-blue hover:bg-ups-yellow hover:text-ups-dark transition-all shadow-xl rounded-full items-center justify-center"
